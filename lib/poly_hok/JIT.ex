...
defmodule JIT do
  ...
  def compile_function({:anon,fname,code,type}) do
   ...
   # IO.inspect "inf_types: #{inspect inf_types}"
    ...
    cuda_body = OCLPolyHok.CudaBackend.gen_cuda_jit(body,inf_types,param_vars,"module",MapSet.new())
  ...
  case Macro.escape(kernel) do
    {:&, [],[{:/, [], [{{:., [], [_module, kernelname]}, [no_parens: true], []}, _nargs]}]} ->
             ...
             # IO.puts module
              ...
              OCLPolyHok.load_kernel_nif(to_charlist("Elixir.#{kernelname}"),to_charlist("#{kernelname}"))

    _ -> raise "OCLPolyHok.build: invalid kernel"
  end
end
...
def gen_jit_kernel_load({:defk,_,[header,[body]]}, is_typed, inf_types) do
 ...
 # IO.puts code
  ...
  {result, errcode} = System.cmd("nvcc",
    ...
    if ((errcode == 1) || (errcode ==2)) do raise "Error when JIT compiling .cu file generated by OCLPolyHok: #{kname}\n #{result}" end
    IO.puts "antes"
