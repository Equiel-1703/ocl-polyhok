defmodule OCLPolyHok do
  ...
  # The phok macro is used to process lambda functions for execution on the GPU.
  defmacro phok({:fn, aa, [{:->, bb, [para, body]}]}) do
    body = OCLPolyHok.CudaBackend.add_return(body)
    name = "anon_" <> OCLPolyHok.CudaBackend.gen_lambda_name()
  ...
  # This is possibly a static/old defmodule? It doesn't appear to be used anywhere else, so I'll ignore it.

  # Yeah, I confirmed with DuBois. This is from an old implementation of PolyHok. I'll ignore it.
  defmacro defmodule_st(header, do: body) do
    # IO.inspect header
    # IO.inspect body
    {:__aliases__, _, [module_name]} = header

    code = OCLPolyHok.CudaBackend.compile_module(module_name, body)
    ...
    if errcode == 1 || errcode == 2 do
      raise "Error when compiling .cu file generated by OCLPolyHok:\n#{result}"
    end

    ast_new_module = OCLPolyHok.CudaBackend.gen_new_module(header, body)
  ...
  def get_gmatrex({:nx, type, shape, name, ref}) do
    ...
    %Nx.Tensor{
      data: %Nx.BinaryBackend{state: get_matrex_nif(ref, rows, cols)},
      type: type,
      shape: shape,
      names: name
    }
  ...
  def load(kernel) do
    case Macro.escape(kernel) do
      {:&, [], [{:/, [], [{{:., [], [_module, kernelname]}, [no_parens: true], []}, _nargs]}]} ->
        ...
        # raise "hell"
        # module_name=String.slice("#{module}",7..-1//1) # Eliminates Elixir.
        OCLPolyHok.load_kernel_nif(to_charlist("Elixir.App"), to_charlist("#{kernelname}"))
  ...
  def load_fun(fun) do
    case Macro.escape(fun) do
      {:&, [], [{:/, [], [{{:., [], [_module, funname]}, [no_parens: true], []}, _nargs]}]} ->
        ...
        OCLPolyHok.load_fun_nif(to_charlist("Elixir.App"), to_charlist("#{funname}"))

      _ ->
        raise "OCLPolyHok.invalid function"
    end
  end

  def load_lambda_compilation(_module, lambda, type) do
    # {:anon, lambda, OCLPolyHok.load_fun_nif(to_charlist(module),to_charlist(lambda)), type}
    {:anon, lambda, type}
  end

  def load_lambda(lambda) do
    OCLPolyHok.load_fun_nif(to_charlist("Elixir.App"), to_charlist(lambda))
  end
  ...
  # I couldn't find sample codes that use this function, so I'll ignore it for porting OCLPolyHok.
