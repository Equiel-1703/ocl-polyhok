defmodule OCLPolyHok.CudaBackend do
  ########
  #Generating a new module in pure Elixir to substitute the OCLPolyHok module in the Elixir program
  ################
...
############################
  ...
  #############################################
  ##### Compiling the definitions in a OCLPolyHok module
  #####################
  ...
  def compile_kernel(_module_name,{:defk,_,[header,[body]]}, type_def,module) do
    ...
    inf_types = OCLPolyHok.TypeInference.type_check(delta,body)
   ...
   # raise "hell"
    ...
    accessfunc = OCLPolyHok.CudaBackend.gen_kernel_call(fname,length(para),Enum.reverse(types_para))
    "\n" <> k <> "\n\n" <> accessfunc
  end
  ...
  def gen_lambda(module,lambda) do
    ...
    if ((errcode == 1) || (errcode ==2)) do raise "Error when compiling .cu file generated by OCLPolyHok:\n#{result}" end
