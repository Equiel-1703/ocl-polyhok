...
OCLPolyHok.defmodule NN do
  ...
  def reduce(ref, acc, f) do
    ...
    size = l*c
     ...
     OCLPolyHok.spawn(&NN.reduce_kernel/4,{numberOfBlocks,1,1},{threadsPerBlock,1,1},[ref, result_gpu, f, size])
     result_gpu
 end
 ...
 if (cacheIndex == 0) do
  ...
  def map_step_2para_1resp(d_array,step, par1, par2, size, f) do
    type = OCLPolyHok.get_type_gnx(d_array)
      ...
      OCLPolyHok.spawn(&NN.map_step_2para_1resp_kernel/7,{size,1,1},{1,1,1},[d_array,distances_device,step,par1,par2,size,f])
      distances_device
  end
...
_r= OCLPolyHok.new_gnx(data_set_host)
