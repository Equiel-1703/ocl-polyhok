...
def map2xy2D1p(arr1,arr2,par,resp,size,f) do
  ...
  OCLPolyHok.spawn(&MM.map2xy2D_kernel/6,{grid_cols,grid_rows,1},{block_size,block_size,1},[arr1,arr2,par,resp,size,f])
end
def comp2xy2D1p(arr1,arr2,par,size1,size2,f) do


    result_gpu = OCLPolyHok.new_gnx(size1,size2,OCLPolyHok.get_array_type(arr1))
...
_result = OCLPolyHok.gpufor x <- 0..m, y <- 0..m, mat1, mat2,m do
            sum = 0
